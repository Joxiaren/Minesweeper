...
bool printfMinefield(int n, int m, int a[n][m])
{
    int j,jj;
    char  blank[2] = "";
    if(n >= 10 || m >= 10) strcpy(blank, " ");

    bool win = true;
    printf("   %s",blank);
    for(j = 0; j < m; j++)
    {
        if(j < 9) printf("%d %s",j+1,blank);
        else printf("%d%s",j+1,blank);
    } 
    printf("\n\n");
    ...
    for(j = 0; j < n; j++)
    {
        ...
        for(jj = 0; jj < m; jj++)
        {
            if(a[j][jj] >= 10) printf("%d %s",a[j][jj] - 10,blank);
            else if(a[j][jj] == 9) printf("* %s",blank);
            else if(a[j][jj] >= 0)
            {
                printf("# %s",blank);
                win = false;
